...
int32_t msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	//lwl add for cit		
			...
			CDBG("%s: L(%d) start to set CFG_SET_FACTORY_TEST_PARAMS \n", __func__, __LINE__);
			
			if (NULL == s_ctrl->func_tbl->odmm_sensor_config){
				
				pr_err("%s:  L(%d)   ( No Support  set CFG_SET_FACTORY_TEST_PARAMS ) !! \n", __func__, __LINE__);
				rc = -EFAULT;
				break;
			
			 }
			
			 rc = s_ctrl->func_tbl->odmm_sensor_config(s_ctrl, cdata);
...
#if 0  //modify by lwl for power sequence control
	rc = msm_camera_request_gpio_table(data, 1);
	if (rc < 0) {
		pr_err("%s: request gpio failed\n", __func__);
		goto request_gpio_failed;
...
int32_t msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	msm_cam_clk_enable(&s_ctrl->sensor_i2c_client->client->dev,
		cam_clk_info, &s_ctrl->cam_clk, ARRAY_SIZE(cam_clk_info), 0);
#if 0 //lwl modify for power sequence control
	msm_camera_config_gpio_table(data, 0);
	msm_camera_enable_vreg(&s_ctrl->sensor_i2c_client->client->dev,
		s_ctrl->sensordata->sensor_platform_info->cam_vreg,
		s_ctrl->sensordata->sensor_platform_info->num_vreg,
