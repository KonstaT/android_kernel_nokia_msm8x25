...
#define APANIC_MMC_WRITE_BY_CP
#define SMEM_ID_VENDOR1_SIZE	(64*1024)

static void *smem_ram = NULL;		//smem_ram start address
static int smem_size = 0;   //smem_ram length
static int smem_off = 0;

static void apanic_smem_init(void)
{
	smem_ram = smem_get_entry(SMEM_ID_VENDOR1,&smem_size);
	memset(smem_ram, 0, smem_size);
}

static int apanic_write_console_smem(unsigned int offset)
{
	int rc;
	int saved_oip;
	int last_chunk=0;
	int idx=0;
	int	len2write = smem_size-smem_off;

	while (!last_chunk) {
		saved_oip = oops_in_progress;
		oops_in_progress = 1;

		rc = log_buf_copy(smem_ram+offset, idx, len2write);
		if (rc <= 0)
			break;
		...
		if(smem_off >= smem_size)
			break;

		len2write = smem_size - smem_off;
	}
...
static void apanic_mmc_logbuf_dump(void)
{
	...
	smem_off += 1024;
	hdr = (struct panic_header *) smem_ram;

	hdr->magic = PANIC_MAGIC;
	hdr->version = PHDR_VERSION;

	hdr->console_offset = 1024;
	hdr->console_length = apanic_write_console_smem(1024);

	hdr->app_threads_offset = 0;
	hdr->app_threads_length = 0;

	hdr->threads_offset = 0;
	hdr->threads_length = 0;
...
void apanic_mmc_parition_add(struct hd_struct *part)
{
	...
	if (!strcmp(dev_name(dev), "mmcblk0")) {
		pr_info("apanic_mmc: dev_name not match: %s\n", dev_name(dev));
		return;
	}

	if (24 != part->partno){
		pr_warn("apanic_mmc: partno not match: %d\n", part->partno);
		return;
	}
